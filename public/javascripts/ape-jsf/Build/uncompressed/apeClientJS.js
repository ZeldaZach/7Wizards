...
APE.Client.prototype.load = function(config){
    ...
    if (tmp) {
        ...
        if(tmp.frequency > 4 && APE.Config.isDebug) {
            config.frequency = 0
        } else {
    ...
    if(Prototype.Browser.IE) {
        iframe = document.getElementById("ape_wizards_ie");
    } else {
    ...
    var ape_init = function() {
        var interval = window.setInterval(function(){
            if (iframe && iframe.contentWindow && iframe.contentWindow.APE) {
                try {
    ...
    if(Prototype.Browser.IE) {
        window.setTimeout(ape_init, 10000); //Sometimes IE fire the onload event, but the iframe is not loaded -_-
    } else {
...
/***
 * APE JSF Setup
 * Shoul be implemented in chat init
 */
//APE.Config.baseUrl = 'http://www.7wizards.com:1024/javascripts/ape-jsf'; //APE JSF
//APE.Config.domain = 'auto';
//APE.Config.server = 'www.7wizards.com:6969'; //APE server URL
